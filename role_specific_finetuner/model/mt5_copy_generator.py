from transformers import MT5ForConditionalGeneration, MT5Model, AutoConfig
...
class MT5CopyGenerator(MT5ForConditionalGeneration):
    """
    ...
    def _compute_output_dist(
        ...
        p_vocab_decoder = self.lm_head(decoder_outputs)
        p_vocab_context = self.lm_head(context_vectors)
        ...
        pgen_decoder_prev_output = self.pgen_decoder_prev_output_layer(
            MT5CopyGenerator._shift_right_one_pad(decoder_outputs)
        )
        p_gen = nn.Sigmoid()(
...
if __name__ == "__main__":
    ...
    model = MT5CopyGenerator.from_pretrained(model_name, config=config)